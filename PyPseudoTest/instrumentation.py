...
class MutantInserter(ast.NodeTransformer):
    ...
    def visit_FunctionDef(self, node):
        ...
        new_body = []
        mutated = False 
        if node.name == 'add' and 'skip-addition' in self.mutants and not mutated:
        ...
        if node.name == 'subtract' and 'invert-subtraction' in self.mutants and not mutated:
        ...
        if node.name == 'multiply' and 'skip-multiplication' in self.mutants and not mutated:
            new_body.append(ast.parse(
        ...
        if node.name == 'divide' and 'skip-division' in self.mutants and not mutated:
            new_body.append(ast.parse(
