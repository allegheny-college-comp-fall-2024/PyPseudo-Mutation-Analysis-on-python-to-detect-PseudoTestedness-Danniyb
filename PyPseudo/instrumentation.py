...
class MutantInserter(ast.NodeTransformer):
    def __init__(self, plugin_name, mutants):
        ...
        self.mutated_functions = set()  # Track functions with mutations

    def visit_FunctionDef(self, node):
        # Track if a mutation has already been applied to this function
        mutation_applied = False
        
        # Process the function's name and check if mutation logic should be inserted
        new_body = []
        
        if node.name == 'add' and 'skip-addition' in self.mutants and 'add' not in self.mutated_functions:
        ...
        if node.name == 'subtract' and 'invert-subtraction' in self.mutants and 'subtract' not in self.mutated_functions:
            mutation_applied = True
            new_body.append(ast.parse(
        ...
        if node.name == 'multiply' and 'skip-multiplication' in self.mutants and 'multiply' not in self.mutated_functions:
            mutation_applied = True
            new_body.append(ast.parse(
        ...
        if node.name == 'divide' and 'skip-division' in self.mutants and 'divide' not in self.mutated_functions:
            mutation_applied = True
            new_body.append(ast.parse(
        ...
        # If mutation logic was applied, track the function to avoid re-mutation
        if mutation_applied:
            self.mutated_functions.add(node.name)

        # Always append the original body, whether mutated or not
...
def run_instrumentation(input_file, mutant_file):
    ...
    # Overwrite the original file with the mutated code
    with open(input_file, 'w') as f:
        f.write(mutated_code)

# Usage example
run_instrumentation('simplePro/calculator.py', 'mutants.json')